...
const FounderCard = ({ image, name, position, quote, linkedin, instagram, github, phone }) => (
    <div className="bg-paper rounded-3xl p-12 transform transition-all duration-500 relative border-4 border-red-200 max-w-2xl mx-auto">
        {/* Award Icon */}
        ...
        <div className="absolute top-6 right-8">
            <Quote className="w-12 h-12 text-red-700 bg-white rounded-full p-3" />
        </div>

        <div className="relative w-80 h-80 mx-auto mb-8 overflow-hidden">
            <div className="absolute inset-0 bg-paper" />
            <div className="absolute inset-2 bg-white shadow-inner" />
 <div className="absolute inset-4 overflow-hidden ring-4 ring-red-100">
                <Image
                    src={image}
                    alt={name}
                    fill
        ...
        <div className="flex justify-center space-x-6 mt-6">
            <a href={linkedin} target="_blank" rel="noopener noreferrer" className="group">
                <div className="bg-white rounded-full p-3 transition-all duration-300 group-hover:bg-red-50 border border-red-100">
        ...
        <div className="absolute bottom-4 right-4">
            <div className="bg-red-800 text-white px-4 py-2 rounded-full text-sm font-semibold border border-red-700">
                Est. 2023
            </div>
        </div>
    </div>
...
const AlumniLeadershipSection = ({ leaders, sectionTitle, isFounderSection = false }) => {
    return (
        <div className={`min-h-screen bg-paper py-20 px-4 ${isFounderSection ? 'bg-paper ' : ''}`}>
            <div className="container mx-auto">
                ...
                {isFounderSection ? (
                    ...
                    <div className={`max-w-7xl mx-auto ${ leaders.length <= 2 ? 'flex flex-col items-center gap-8' : 'grid grid-cols-1 md:grid-cols-3 gap-8' }`}>
                        {leaders.map((leader, index) => (
                            <LeaderCard
                                key={index}
                                name={leader.name}
