            ...
            >
                <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                    <Link href="/" className="flex items-center space-x-3">
                        ...
                        <span className={`text-2xl font-bold ${scrolled ? "text-red-500" : "text-red-600"}`}>Conventus</span>
                    </Link>
                    <nav className="hidden md:flex space-x-6">
                        {navItems.map((item) => (
                            <motion.div key={item.href} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                                <Link href={item.href} className={`text-lg font-semibold ${scrolled ? 'text-gray-300 hover:text-red-500' : 'text-red-600 hover:text-red-400'}`}>
                                    {item.label}
                                </Link>
                            </motion.div>
                        ))}
